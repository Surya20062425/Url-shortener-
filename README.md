...
Here's a comprehensive URL shortener with:
- **SQLite database** for persistence
- **Hash-based shortening** (collision-resistant)
...
|---------|---------------|
| **Shortening** | SHA-256 hash + Base62 encoding (collision-resistant) |
| **Storage** | SQLite with proper indexing |
| **Analytics** | Click tracking with IP, UA, referrer |
| **Expiration** | Automatic cleanup of expired URLs |
| **Rate Limiting** | In-memory sliding window (100 req/hour) |
| **Custom Codes** | Optional user-defined short codes |
...
**2. Server Mode (requires Flask):**
```bash
pip install flask
python url_shortener.py --server
```
...
# Redirect
curl -I http://localhost:5000/mylink
```

The code uses professional patterns: thread-safe database connections, proper error handling, input validation, and clean separation of concerns. The Base62 encoding produces short, readable URLs while maintaining uniqueness through hash-based generation with collision detection.
